...
class VotacionAdapter(
    ...
    override fun onBindViewHolder(holder: Vh, position: Int) {
        ...
        if (active) {
            val votedAlready = voted[item.id] == true
            holder.buttonVote.isEnabled = !votedAlready
            if (votedAlready) {
                holder.buttonVote.backgroundTintList = ColorStateList.valueOf(
                    holder.itemView.context.getColor(R.color.disabled_gray)
                )
            } else {
